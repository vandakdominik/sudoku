...
document.querySelector("#sudoku-reset").addEventListener("click", reset);
...
function createGrid() {
  ...
  for (let i = 0; i < 9; i++) {
    for (let j = 0; j < 9; j++) {
      ...
      if (odd) {
        document
          .querySelector(`#sudoku-input-js-${index}`)
          .classList.add("sudoku-odd");
      } else {
        document
          .querySelector(`#sudoku-input-js-${index}`)
          .classList.add("sudoku-even");
      }
    }
...
function reset() {
  ...
  statusElem.innerText = "Enter values";
}
